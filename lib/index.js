const { Statement, ReadResult } = require('./array')
const builder = require('./builder')
const clients = require('./clients')
const is = require('./is')
const { pagination, paginationBefore, paginationAfter } = require('./pagination')
...
function xSQL(hostId = xSQL.options.defaultHost, skip = false) {
  ...
  this._runCommand = async function (cmd, table, data) {
    ...
    try {
      ...
      if (is.array(readResult)) {
        ...
        // Relationshop
        if (!is.empty(this._state.relation)) {
   ...
   * @returns {Promise<OkPacket>}
   */
  this.update = function (table, data, options) {
   ...
   */
  this.batchInsert = function (table, data, options) {
