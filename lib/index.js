 ...
 * @returns {nSQL}
 */
function nSQL(hostId = nSQL.options.defaultHost, skip = false) {
  // Return new instance automatically
  if (!(this instanceof nSQL) && !skip) return new nSQL(hostId)

  /** @type {HostOptions} */
  this.host = nSQL.hosts[hostId]
  this.hostId = hostId
  this.conn = null
  this.isTransaction = false
  this.client = nSQL.getClient(this.host.client)
  // const self = this
  ...
  /**
   * Take a "snapshot" of the nSQL instance, returning a new instance.
   * @returns {nSQL}
   */
  this.clone = function () {
    const clone = nSQL(this.hostId)
  ...
  // Transaction
  this.transaction = transaction(nSQL, hostId)
   ...
   * Split Example:
   * ```js
   * const instance = nsql().where({ id: 1 })
...
nSQL.clients = clients

/**
 * Initializing the Library
 * @param {defaultOptions} options
 */
nSQL.init = function (options) {
...
nSQL.getClient = function (client) {
  return nSQL.clients[client]
}
