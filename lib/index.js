...
function xsql(hostId = xsql.options.defaultHost, skip = false) {
  ...
  this.leftJoin = builder.leftJoin
  this.rightJoin = builder.rightJoin
  ...
  this.toStatement = function (cmd, table, data, options) {
    ...
    if (is.notEmpty(jsonKeys) && is.defined(data)) {
      if (is.plainObject(data)) {
        data = objectToJson(data, jsonKeys, jsonMap)
      }
      if (is.array(data)) {
        data = data.map((d) => objectToJson(d, jsonKeys, jsonMap))
      }
    }
...
xsql.clients = {
  /** @type {import('../clients/mysql')} */
  mysql: null,
  /** @type {import('../clients/mysql2')} */
  mysql2: null,
  /** @type {import('../clients/database-js')} */
  databaseJs: null,
}
