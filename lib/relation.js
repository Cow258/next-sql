 ...
 * @property {(q: xsql, targetIds: [], row => ReadResult) => {}} [override]
 */
...
function mapHelper(input) {
  ...
  if (!currentKey || !targetTable || !targetKey)
    throw new Error(`Relation mapper is invalid!\nExpected: <currentKey>:<foreignTable>.<foreignKey>,\nbut received: ${input}`)
...
async function _relationFetch(currentRows, xsql) {
    ...
    defers.push((async () => {
      ...
      if (relation.override) {
        if (relation.type === 'fromOne') {
          targetResult = await relation.override(_sql, currentRows.map(currentRow => currentRow[relation.currentKey]), currentRows)
        } else {
          const targetIds = [...new Set(relation.currentIds)]
          if (!targetIds.length) return
          targetResult = await relation.override(_sql, targetIds, currentRows)
        }
      } else {
        if (relation.select) {
  ...
  if (this._state.log) {
    this.client.logger(`${chalk.underline(`${Date.now() - startAt}ms`)} xsql._relationFetch()`)
  }
  return errList
}
